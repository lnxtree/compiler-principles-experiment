...
IRBuild_ptn = '"{}" "-ir" "{}" > "{}" '
ExeGen_ptn = '"clang" "-o" "{}" "{}" "./lib/sylib.c"'
Exe_ptn = '"{}"'
...
def eval(EXE_PATH, TEST_BASE_PATH, optimization):
    ...
    for case in testcases:
        ...
        print(IRBuild_ptn.format(EXE_PATH, SY_PATH, LL_PATH))

        IRBuild_result = subprocess.run(IRBuild_ptn.format(EXE_PATH, SY_PATH, LL_PATH),
                                        shell=True,
...
if __name__ == "__main__":
    ...
    optimization = NULL     # NULL / -O2 / -O -cse / -O -dce / -O -cp
    for TEST_BASE_PATH in TEST_DIRS:
        testcases = {}  # { name: need_input }
        EXE_PATH = os.path.abspath('./src/build/compiler')
        testcase_list = list(
